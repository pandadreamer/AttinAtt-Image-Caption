...
class DataLoader(data.Dataset):
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        for i in range(batch_size):
            ...
            tmp_fc = tmp_fc.reshape(-1, 2048)
            tmp_att = tmp_att.reshape(-1, 49, 2048)
        ...
        #print("fc_batch"+str(fc_batch[0].shape))
        #print("att_batch"+str(att_batch[0].shape))
        data = {}
        data['fc_feats'] = np.stack(sum([[_]*seq_per_img for _ in fc_batch], []))
        # merge att_feats
        max_att_len = max([_.shape[0] for _ in att_batch])
        data['att_feats'] = np.zeros([len(att_batch)*seq_per_img, max_att_len, att_batch[0].shape[1], att_batch[0].shape[2]], dtype = 'float32')
        for i in range(len(att_batch)):
            data['att_feats'][i*seq_per_img:(i+1)*seq_per_img, :att_batch[i].shape[0]] = att_batch[i]
        #print("att_feats"+str(data['att_feats'].shape))
    ...
    def __getitem__(self, index):
        ...
        if self.use_att:
            ...
            # Reshape to K x C
            #att_feat = att_feat.reshape(-1, 7, 7, 2048)
