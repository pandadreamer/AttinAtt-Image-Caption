...
class AttModel(CaptionModel):
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        att_feats = self.attinatt(att_feats)
        
        att_feats, att_masks = self.clip_att(att_feats, att_masks)

        # embed fc and att feats
        #fc_feats = fc_feats.reshape(49,2048)
...
class AttinAtt(nn.Module):
    def __init__(self, opt):
        super(AttinAtt, self).__init__()
        self.att_feat_size = opt.att_feat_size

        self.beta_net = nn.Linear(self.att_feat_size, 1)

    def forward(self, att_feats):
        att_size = att_feats.numel() // att_feats.size(0) // att_feats.size(1) // att_feats.size(-1)

        att = att_feats.view(-1, att_size, self.att_feat_size)      

        dot = F.tanh(att)
        dot = self.beta_net(dot)
        dot = dot.view(-1, att_size)

        weight = F.softmax(dot, dim=1)
        att_res = torch.bmm(weight.unsqueeze(1), att).squeeze(1).reshape(att_feats.size(0), att_feats.size(1), self.att_feat_size)
        return att_res
